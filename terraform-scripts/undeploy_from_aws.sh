...
# BANK SYSTEM
cd bank
terraform init && terraform destroy -var="bank_db_username=$db_username" -var="bank_db_password=$db_password" -auto-approve || exit 1

# STORE SYSTEM
cd ../store
terraform init && terraform destroy -var="store_db_username=$db_username" -var="store_db_password=$db_password" -auto-approve || exit 1

# PUBLICITAKI SYSTEM
cd ../publicitaki
terraform init && terraform destroy -var="pub_db_username=$db_username" -var="pub_db_password=$db_password" -auto-approve || exit 1
